...
const State = {
  ...
  kbNotif: {},
  activityLog: {},
  // Atividades: Kanban / Lista / Calendário
  atvColunas: {},
  atvTarefas: {},
  atvCategorias: {},
  atvLixeira: {}
...
const App = {
  ...
  abrirSecao(btn) {
    ...
    if (alvo === 'tab-atividades')  _run('atvRender', () => App.atvRender?.());
  ...
  initListeners() {
    ...
    liga('atvColunas', v => {
      State.atvColunas = v || {};
      App._atvSeedColunas?.();
      if (document.getElementById('tab-atividades')?.classList.contains('active')) App.atvRender?.();
    });
    liga('atvTarefas', v => {
      State.atvTarefas = v || {};
      if (document.getElementById('tab-atividades')?.classList.contains('active')) App.atvRender?.();
    });
    liga('atvCategorias', v => {
      State.atvCategorias = v || {};
      if (document.getElementById('tab-atividades')?.classList.contains('active')) App._atvPopularSelects?.();
    });
    liga('atvLixeira', v => {
      State.atvLixeira = v || {};
      App._atvAtualizarBadgeLixeira?.();
    });
...
/* ══════════════════════════════════════════════════════════
   ATIVIDADES — Kanban / Lista / Calendário
   Módulo novo e isolado (prefixo atv-/_atv). Reaproveita só leitura
   de State.units (Setores) e State.admins (Responsável/Revisor) —
   não escreve neles. Nada aqui toca Financeiro/Inventário/
   Relatórios/Gerador de PDFs.
   ══════════════════════════════════════════════════════════ */
Object.assign(App, {
  _atvView: 'kanban',
  _atvListaSub: 'cards',
  ...
  _atvCalRef: new Date(),
  _atvBusca: '',
  _atvModoVisao: 'minhas',
  _atvFiltroRevisor: '',
  _atvFiltros: { setor: '', categoria: '', prioridade: '' },
  _atvOrdenacaoTabela: { campo: 'fim', dir: 'asc' },
  _atvSubtarefasEdit: [],
  _atvDragId: null,
  _atvSeedFeito: false,
  _atvSeedCatFeito: false,
  _ATV_CORES_COL: ['#64748b', '#3a7ee8', '#e8830a', '#7c52d4', '#1db87a', '#d94040', '#00b8a2', '#2368d4'],

  /* Semeia as 6 colunas padrão só se o nó estiver vazio — nunca roda de
     novo depois (mesmo que o usuário apague todas), pra não ressuscitar
     coluna apagada de propósito. */
  _atvSeedColunas() {
    if (App._atvSeedFeito) return;
    App._atvSeedFeito = true;
    if (Object.keys(State.atvColunas || {}).length) return;
    [['Novas Demandas', '#64748b'], ['Em análise', '#e8830a'], ['Em andamento', '#3a7ee8'],
     ['Em Revisão', '#e8830a'], ['Concluído', '#1db87a'], ['Cancelado', '#d94040']]
      .forEach(([nome, cor], i) => DB.push('atvColunas', { nome, cor, ordem: i }));
  },
  _atvSeedCategorias() {
    if (App._atvSeedCatFeito) return;
    App._atvSeedCatFeito = true;
    if (Object.keys(State.atvCategorias || {}).length) return;
    ['Estrutura', 'Configuração', 'Orçamentos', 'Projeto', 'Servidor', 'Equipe T.I']
      .forEach(nome => DB.push('atvCategorias', nome));
  },

  /* ── Troca de vista (Kanban/Lista/Calendário) ── */
  atvSetView(view) {
    App._atvView = view;
    document.querySelectorAll('.atv-view-btn').forEach(b => b.classList.toggle('active', b.dataset.view === view));
    document.querySelectorAll('.atv-view').forEach(v => v.classList.remove('active'));
    document.getElementById('atv-view-' + view)?.classList.add('active');
    App.atvRender();
  },
  atvRender() {
    App._atvSeedColunas();
    App._atvSeedCategorias();
    App._atvPopularSelects();
    if (App._atvView === 'kanban')          App.atvRenderKanban();
    else if (App._atvView === 'lista')      App.atvRenderLista();
    else if (App._atvView === 'calendario') App.atvRenderCalendario();
    App._atvAtualizarBadgeLixeira();
  },

  /* ── Filtros (busca, revisor, minhas/todas, chips avançados) ── */
  atvBusca(v) { App._atvBusca = v; App.atvRender(); },
  atvFiltroRevisor(v) { App._atvFiltroRevisor = v; App.atvRender(); },
  atvSetModoVisao(v) { App._atvModoVisao = v; App.atvRender(); },
  atvToggleFiltro() { document.getElementById('atv-filtro-panel')?.classList.toggle('hidden'); },
  atvLimparFiltros() {
    App._atvFiltros = { setor: '', categoria: '', prioridade: '' };
    App._atvPopularSelects();
    App.atvRender();
  },
  _atvToggleFiltroChip(campo, val) {
    App._atvFiltros[campo] = App._atvFiltros[campo] === val ? '' : val;
    App._atvPopularSelects();
    App.atvRender();
  },
  _atvTarefasFiltradas() {
    const busca = (App._atvBusca || '').toLowerCase().trim();
    const eu = State.adminUser || '';
    return Object.entries(State.atvTarefas || {})
      .filter(([, t]) => !!t)
      .map(([id, t]) => ({ id, ...t }))
      .filter(t => {
        if (App._atvModoVisao === 'minhas' && eu &&
            t.responsavel !== eu && t.revisor !== eu && t.criadoPor !== eu) return false;
        if (App._atvFiltroRevisor && t.revisor !== App._atvFiltroRevisor) return false;
        if (App._atvFiltros.setor      && t.setor      !== App._atvFiltros.setor)      return false;
        if (App._atvFiltros.categoria  && t.categoria  !== App._atvFiltros.categoria)  return false;
        if (App._atvFiltros.prioridade && t.prioridade !== App._atvFiltros.prioridade) return false;
        if (busca && !(t.titulo || '').toLowerCase().includes(busca)) return false;
        return true;
      });
  },

  /* Repovoa selects/chips com Setores (State.units), Responsável/Revisor
     (State.admins), Categorias e Colunas — chamado sempre que um desses
     nós muda, ou ao trocar de vista, pra nunca ficar desatualizado. */
  _atvPopularSelects() {
    const manterValor = (el, html) => { if (!el) return; const v = el.value; el.innerHTML = html; el.value = v; };

    const setores = Object.entries(State.units || {}).sort((a, b) => String(a[1]).localeCompare(String(b[1])));
    const optSetor = '<option value="">—</option>' + setores.map(([id, nome]) => `<option value="${id}">${nome}</option>`).join('');
    manterValor(document.getElementById('atv-task-setor'), optSetor);

    const admins = Object.entries(State.admins || {}).sort((a, b) =>
      String((a[1] && a[1].nome) || a[0]).localeCompare(String((b[1] && b[1].nome) || b[0])));
    const optAdmin = (placeholder) => `<option value="">${placeholder}</option>` +
      admins.map(([login, dados]) => `<option value="${login}">${(dados && dados.nome) || login}</option>`).join('');
    manterValor(document.getElementById('atv-task-responsavel'), optAdmin('—'));
    manterValor(document.getElementById('atv-task-revisor'), optAdmin('—'));
    manterValor(document.getElementById('atv-filtro-revisor'), optAdmin('Filtrar por Revisor'));

    const cats = Object.entries(State.atvCategorias || {}).sort((a, b) => String(a[1]).localeCompare(String(b[1])));
    const optCat = '<option value="">—</option>' + cats.map(([id, nome]) => `<option value="${id}">${nome}</option>`).join('');
    manterValor(document.getElementById('atv-task-categoria'), optCat);

    const cols = Object.entries(State.atvColunas || {}).sort((a, b) => (a[1].ordem || 0) - (b[1].ordem || 0));
    const selCol = document.getElementById('atv-task-coluna');
    if (selCol) { const v = selCol.value; selCol.innerHTML = cols.map(([id, c]) => `<option value="${id}">${c.nome}</option>`).join(''); if (v) selCol.value = v; }

    const wrapSetor = document.getElementById('atv-filtro-setores');
    if (wrapSetor) wrapSetor.innerHTML = setores.length
      ? setores.map(([id, nome]) => `<button type="button" class="atv-chip${App._atvFiltros.setor === id ? ' active' : ''}" onclick="App._atvToggleFiltroChip('setor','${id}')">${nome}</button>`).join('')
      : '<span style="font-size:.78rem;color:var(--ink-300);">Nenhum setor cadastrado</span>';
    const wrapCat = document.getElementById('atv-filtro-categorias');
    if (wrapCat) wrapCat.innerHTML = cats.length
      ? cats.map(([id, nome]) => `<button type="button" class="atv-chip${App._atvFiltros.categoria === id ? ' active' : ''}" onclick="App._atvToggleFiltroChip('categoria','${id}')">${nome}</button>`).join('')
      : '<span style="font-size:.78rem;color:var(--ink-300);">Nenhuma categoria</span>';
    const wrapPrio = document.getElementById('atv-filtro-prioridades');
    if (wrapPrio) wrapPrio.innerHTML = ['P1', 'P2', 'P3', 'P4']
      .map(p => `<button type="button" class="atv-chip${App._atvFiltros.prioridade === p ? ' active' : ''}" onclick="App._atvToggleFiltroChip('prioridade','${p}')">${p}</button>`).join('');
  },

  /* ── Helpers ── */
  _atvNomeAdmin(login) {
    if (!login) return '';
    const rec = State.admins && State.admins[login];
    return (rec && rec.nome) || login;
  },
  _atvFmtData(iso) {
    if (!iso) return '';
  ...
  _atvColunaFinalId() {
    const entry = Object.entries(State.atvColunas || {}).find(([, c]) => /conclu[ií]d|cancelad/i.test(c.nome || ''));
    return entry ? entry[0] : null;
  },

  /* ── KANBAN ── */
  atvRenderKanban() {
    const board = document.getElementById('atv-board');
    if (!board) return;
    const tarefas = App._atvTarefasFiltradas();
    const cols = Object.entries(State.atvColunas || {}).sort((a, b) => (a[1].ordem || 0) - (b[1].ordem || 0));
    if (!cols.length) { board.innerHTML = '<div class="atv-lista-empty">Nenhuma coluna criada ainda. Clique em "Adicionar coluna".</div>'; return; }
    board.innerHTML = cols.map(([colId, col]) => {
      const doColuna = tarefas.filter(t => t.colunaId === colId).sort((a, b) => (a.ordem || 0) - (b.ordem || 0));
      return `
      <div class="atv-col" data-col="${colId}" ondragover="App._atvDragOverCol(event,'${colId}')" ondragleave="App._atvDragLeaveCol(event)" ondrop="App._atvDropCol(event,'${colId}')">
        <div class="atv-col-header">
          <span class="atv-col-dot" style="background:${col.cor || '#64748b'}"></span>
          <span class="atv-col-nome">${col.nome}</span>
          <span class="atv-col-count">${doColuna.length}</span>
          <span class="atv-col-acts">
            <button onclick="App.atvAbrirColuna('${colId}')" title="Editar coluna">${App._svg('pencil')}</button>
          </span>
        </div>
        <div class="atv-col-body">
          ${doColuna.length ? doColuna.map(t => App._atvCardHTML(t)).join('') : '<div class="atv-col-empty">Sem tarefas</div>'}
        </div>
        <button class="atv-col-add" onclick="App.atvAbrirTask(null,'${colId}')">+ Nova tarefa</button>
      </div>`;
    }).join('');
  },
  _atvCardHTML(t) {
    const pct = App._atvProgresso(t);
    const resp = App._atvNomeAdmin(t.responsavel);
    const setorNome = (State.units && State.units[t.setor]) || '';
    const catNome = (State.atvCategorias && State.atvCategorias[t.categoria]) || '';
    const hoje = new Date().toISOString().slice(0, 10);
    const finalId = App._atvColunaFinalId();
    const atrasada = t.fim && t.fim < hoje && t.colunaId !== finalId;
    return `
    <div class="atv-card" draggable="true" ondragstart="App._atvDragStart(event,'${t.id}')" ondragend="App._atvDragEnd(event)" onclick="App.atvAbrirTask('${t.id}')">
      <div class="atv-card-title">${t.titulo || '(sem título)'}</div>
      <div class="atv-card-progress" style="--pct:${pct}">${pct}%</div>
      ${resp ? `<div class="atv-card-row"><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><circle cx="12" cy="8" r="4" stroke="currentColor" stroke-width="2"/><path d="M4 20c0-4 3.5-7 8-7s8 3 8 7" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>${resp}</div>` : ''}
      ${catNome ? `<div class="atv-card-row"><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><path d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" stroke="currentColor" stroke-width="2"/></svg>${catNome}</div>` : ''}
      ${t.fim ? `<div class="atv-card-row atv-card-date${atrasada ? ' atrasada' : ''}"><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><rect x="3" y="4" width="18" height="17" rx="2" stroke="currentColor" stroke-width="2"/><line x1="3" y1="9" x2="21" y2="9" stroke="currentColor" stroke-width="2"/></svg>${App._atvFmtData(t.fim)}</div>` : ''}
      ${setorNome ? `<div class="atv-card-tags"><span class="atv-tag-setor">${setorNome}</span></div>` : ''}
      <div class="atv-card-footer"><span class="atv-priority atv-priority-${t.prioridade || 'P3'}">${t.prioridade || 'P3'}</span></div>
    </div>`;
  },
  _atvCardComStatusHTML(t) {
    const col = State.atvColunas && State.atvColunas[t.colunaId];
    const selo = col ? `<span class="atv-lista-status-pill" style="background:${col.cor}22;color:${col.cor}">${col.nome}</span>` : '';
    return `<div>${selo}${App._atvCardHTML(t)}</div>`;
  },

  /* ── Drag and drop (nativo, sem lib) ── */
  _atvDragStart(ev, id) {
    App._atvDragId = id;
    ev.target.closest('.atv-card')?.classList.add('dragging');
    ev.dataTransfer.effectAllowed = 'move';
  },
  _atvDragEnd(ev) {
    ev.target.closest('.atv-card')?.classList.remove('dragging');
    document.querySelectorAll('.atv-col.drag-over').forEach(c => c.classList.remove('drag-over'));
  },
  _atvDragOverCol(ev, colId) {
    ev.preventDefault();
    document.querySelector(`.atv-col[data-col="${colId}"]`)?.classList.add('drag-over');
  },
  _atvDragLeaveCol(ev) {
    const col = ev.currentTarget;
    if (col && !col.contains(ev.relatedTarget)) col.classList.remove('drag-over');
  },
  _atvDropCol(ev, colId) {
    ev.preventDefault();
    document.querySelector(`.atv-col[data-col="${colId}"]`)?.classList.remove('drag-over');
    const id = App._atvDragId;
    App._atvDragId = null;
    if (!id) return;
    const t = State.atvTarefas && State.atvTarefas[id];
    if (!t || t.colunaId === colId) return;
    DB.update('atvTarefas/' + id, { colunaId: colId, atualizadoEm: new Date().toISOString() });
    App._logActivity?.('atividades', 'Tarefa movida', `"${t.titulo}" → ${(State.atvColunas[colId] || {}).nome || ''}`);
  },

  /* ── Modal de Tarefa (criar/editar) ── */
  atvAbrirTask(id, colunaIdPadrao) {
    App._atvPopularSelects();
    const t = id ? (State.atvTarefas && State.atvTarefas[id]) : null;
    document.getElementById('atv-task-title').textContent = t ? 'Editar Tarefa' : 'Nova Tarefa';
    document.getElementById('atv-task-id').value = id || '';
    document.getElementById('atv-task-titulo').value = t ? (t.titulo || '') : '';
    document.getElementById('atv-task-desc').value = t ? (t.descricao || '') : '';
    const cols = Object.entries(State.atvColunas || {}).sort((a, b) => (a[1].ordem || 0) - (b[1].ordem || 0));
    const primeiraCol = cols.length ? cols[0][0] : '';
    document.getElementById('atv-task-coluna').value = t ? (t.colunaId || primeiraCol) : (colunaIdPadrao || primeiraCol);
    document.getElementById('atv-task-setor').value = t ? (t.setor || '') : '';
    document.getElementById('atv-task-categoria').value = t ? (t.categoria || '') : '';
    document.getElementById('atv-task-responsavel').value = t ? (t.responsavel || '') : (State.adminUser || '');
    document.getElementById('atv-task-revisor').value = t ? (t.revisor || '') : '';
    document.getElementById('atv-task-prioridade').value = t ? (t.prioridade || 'P3') : 'P3';
    document.getElementById('atv-task-inicio').value = t ? (t.inicio || '') : '';
    document.getElementById('atv-task-fim').value = t ? (t.fim || '') : '';
    App._atvSubtarefasEdit = (t && t.subtarefas) ? JSON.parse(JSON.stringify(t.subtarefas)) : [];
    App._atvRenderSubtarefas();
    document.getElementById('atv-task-del-btn').style.display = t ? '' : 'none';
    document.getElementById('atv-task-modal').classList.remove('hidden');
    setTimeout(() => document.getElementById('atv-task-titulo')?.focus(), 80);
  },
  atvFecharTask() {
    document.getElementById('atv-task-modal').classList.add('hidden');
    App._atvSubtarefasEdit = [];
  },
  atvSalvarTask() {
    const titulo = document.getElementById('atv-task-titulo').value.trim();
    if (!titulo) return toast('Informe o título da tarefa.', 'error');
    const id = document.getElementById('atv-task-id').value;
    const dados = {
      titulo,
      descricao: document.getElementById('atv-task-desc').value.trim(),
      colunaId: document.getElementById('atv-task-coluna').value,
      setor: document.getElementById('atv-task-setor').value,
      categoria: document.getElementById('atv-task-categoria').value,
      responsavel: document.getElementById('atv-task-responsavel').value,
      revisor: document.getElementById('atv-task-revisor').value,
      prioridade: document.getElementById('atv-task-prioridade').value,
      inicio: document.getElementById('atv-task-inicio').value,
      fim: document.getElementById('atv-task-fim').value,
      subtarefas: App._atvSubtarefasEdit || [],
      atualizadoEm: new Date().toISOString()
    };
    if (id) {
      DB.update('atvTarefas/' + id, dados);
      App._logActivity?.('atividades', 'Tarefa editada', titulo);
      toast('Tarefa atualizada.');
    } else {
      dados.criadoEm = new Date().toISOString();
      dados.criadoPor = State.adminUser || '';
      dados.ordem = Date.now();
      DB.push('atvTarefas', dados);
      App._logActivity?.('atividades', 'Tarefa criada', titulo);
      toast('Tarefa criada.');
    }
    App.atvFecharTask();
  },
  atvExcluirTask() {
    const id = document.getElementById('atv-task-id').value;
    if (!id) return;
    App._atvEnviarParaLixeira(id);
    App.atvFecharTask();
  },
  _atvExcluirDireto(id) { App._atvEnviarParaLixeira(id); },
  _atvEnviarParaLixeira(id) {
    const t = State.atvTarefas && State.atvTarefas[id];
    if (!t) return;
    if (!confirm(`Mandar "${t.titulo}" pra lixeira?\n\nDá pra restaurar depois, na Lixeira de Atividades.`)) return;
    DB.push('atvLixeira', { tarefa: t, deletedAt: new Date().toISOString() });
    DB.remove('atvTarefas/' + id);
    App._logActivity?.('atividades', 'Tarefa enviada pra lixeira', t.titulo);
    toast('Tarefa enviada pra lixeira.');
  },

  /* ── Checklist / Subtarefas (dirigem o anel de progresso) ── */
  _atvRenderSubtarefas() {
    const wrap = document.getElementById('atv-subtarefas-list');
    const txt = document.getElementById('atv-progresso-txt');
    const subs = App._atvSubtarefasEdit || [];
    if (txt) {
      const feitas = subs.filter(s => s.feita).length;
      txt.textContent = subs.length ? `(${feitas}/${subs.length} — ${Math.round(feitas / subs.length * 100)}%)` : '';
    }
    if (!wrap) return;
    wrap.innerHTML = subs.map((s, i) => `
      <div class="atv-subtarefa-item${s.feita ? ' feita' : ''}">
        <input type="checkbox" ${s.feita ? 'checked' : ''} onchange="App._atvToggleSubtarefa(${i})">
        <span>${s.texto}</span>
        <button type="button" onclick="App._atvRemoverSubtarefa(${i})">${App._svg('x')}</button>
      </div>`).join('');
  },
  atvAddSubtarefa() {
    const inp = document.getElementById('atv-nova-subtarefa');
    const v = inp.value.trim();
    if (!v) return;
    App._atvSubtarefasEdit = App._atvSubtarefasEdit || [];
    App._atvSubtarefasEdit.push({ texto: v, feita: false });
    inp.value = '';
    App._atvRenderSubtarefas();
  },
  _atvToggleSubtarefa(i) {
    if (!App._atvSubtarefasEdit[i]) return;
    App._atvSubtarefasEdit[i].feita = !App._atvSubtarefasEdit[i].feita;
    App._atvRenderSubtarefas();
  },
  _atvRemoverSubtarefa(i) {
    App._atvSubtarefasEdit.splice(i, 1);
    App._atvRenderSubtarefas();
  },

  /* ── Categoria rápida (a partir do modal de tarefa) ── */
  atvNovaCategoria() {
    document.getElementById('atv-cat-nome').value = '';
    document.getElementById('atv-cat-modal').classList.remove('hidden');
    setTimeout(() => document.getElementById('atv-cat-nome')?.focus(), 80);
  },
  atvSalvarCategoria() {
    const nome = document.getElementById('atv-cat-nome').value.trim();
    if (!nome) return;
    const ref = DB.push('atvCategorias', nome);
    document.getElementById('atv-cat-modal').classList.add('hidden');
    const key = ref.key;
    setTimeout(() => { const sel = document.getElementById('atv-task-categoria'); if (sel && key) sel.value = key; }, 250);
  },

  /* ── Modal de Coluna (criar/editar/excluir) ── */
  atvAbrirColuna(id) {
    const c = id ? (State.atvColunas && State.atvColunas[id]) : null;
    document.getElementById('atv-col-title').textContent = c ? 'Editar Coluna' : 'Nova Coluna';
    document.getElementById('atv-col-id').value = id || '';
    document.getElementById('atv-col-nome').value = c ? (c.nome || '') : '';
    document.getElementById('atv-col-cor').value = c ? (c.cor || App._ATV_CORES_COL[0]) : App._ATV_CORES_COL[0];
    App._atvRenderCoresColuna();
    document.getElementById('atv-col-del-btn').style.display = c ? '' : 'none';
    document.getElementById('atv-col-modal').classList.remove('hidden');
    setTimeout(() => document.getElementById('atv-col-nome')?.focus(), 80);
  },
  _atvRenderCoresColuna() {
    const wrap = document.getElementById('atv-col-cores');
    const atual = document.getElementById('atv-col-cor').value;
    if (!wrap) return;
    wrap.innerHTML = App._ATV_CORES_COL.map(c =>
      `<button type="button" class="atv-cor-swatch${c === atual ? ' active' : ''}" style="background:${c}" onclick="App._atvEscolherCorColuna('${c}')"></button>`).join('');
  },
  _atvEscolherCorColuna(c) {
    document.getElementById('atv-col-cor').value = c;
    App._atvRenderCoresColuna();
  },
  atvFecharColuna() { document.getElementById('atv-col-modal').classList.add('hidden'); },
  atvSalvarColuna() {
    const nome = document.getElementById('atv-col-nome').value.trim();
    if (!nome) return toast('Informe o nome da coluna.', 'error');
    const id = document.getElementById('atv-col-id').value;
    const cor = document.getElementById('atv-col-cor').value;
    if (id) DB.update('atvColunas/' + id, { nome, cor });
    else {
      const max = Math.max(0, ...Object.values(State.atvColunas || {}).map(c => c.ordem || 0));
      DB.push('atvColunas', { nome, cor, ordem: max + 1 });
    }
    App.atvFecharColuna();
  },
  atvExcluirColuna() {
    const id = document.getElementById('atv-col-id').value;
    if (!id) return;
    const temTarefas = Object.values(State.atvTarefas || {}).some(t => t.colunaId === id);
    if (temTarefas) return toast('Mova ou apague as tarefas desta coluna antes de excluí-la.', 'error');
    if (!confirm('Excluir esta coluna?')) return;
    DB.remove('atvColunas/' + id);
    App.atvFecharColuna();
  },

  /* ── Lixeira de Atividades ── */
  atvAbrirLixeira() {
    App._atvRenderLixeira();
    document.getElementById('atv-trash-modal').classList.remove('hidden');
  },
  _atvRenderLixeira() {
    const body = document.getElementById('atv-trash-body');
    if (!body) return;
    const itens = Object.entries(State.atvLixeira || {}).sort((a, b) => new Date(b[1].deletedAt) - new Date(a[1].deletedAt));
    if (!itens.length) { body.innerHTML = '<div class="atv-trash-empty">Lixeira vazia.</div>'; return; }
    body.innerHTML = itens.map(([id, reg]) => `
      <div class="atv-trash-item">
        <div class="atv-trash-item-info">
          <div class="atv-trash-item-nome">${(reg.tarefa && reg.tarefa.titulo) || 'Tarefa'}</div>
          <div class="atv-trash-item-meta">Apagada em ${new Date(reg.deletedAt).toLocaleString('pt-BR')}</div>
        </div>
        <div class="atv-trash-item-acts">
          <button class="btn-ghost" onclick="App.atvRestaurarTask('${id}')">Restaurar</button>
          <button class="btn-delete" onclick="App.atvExcluirDefinitivo('${id}')">Apagar de vez</button>
        </div>
      </div>`).join('');
  },
  atvRestaurarTask(id) {
    const reg = State.atvLixeira && State.atvLixeira[id];
    if (!reg) return;
    DB.push('atvTarefas', reg.tarefa);
    DB.remove('atvLixeira/' + id);
    toast('Tarefa restaurada.');
    setTimeout(() => App._atvRenderLixeira(), 150);
  },
  atvExcluirDefinitivo(id) {
    if (!confirm('Apagar definitivamente? Não dá pra restaurar depois.')) return;
    DB.remove('atvLixeira/' + id);
    setTimeout(() => App._atvRenderLixeira(), 150);
  },
  _atvAtualizarBadgeLixeira() {
    const n = Object.keys(State.atvLixeira || {}).length;
    const badge = document.getElementById('atv-trash-badge');
    const btn = document.querySelector('.atv-trash-btn');
    if (badge) { badge.textContent = n; badge.style.display = n ? '' : 'none'; }
    if (btn) btn.classList.toggle('has-items', n > 0);
  },

  /* ── LISTA (Cards / Tabela / Grupos / Setores) ── */
  atvSetListaSub(sub) {
    App._atvListaSub = sub;
    document.querySelectorAll('.atv-lista-sub .atv-sub-btn').forEach(b => b.classList.toggle('active', b.dataset.sub === sub));
    App.atvRenderLista();
  },
  atvRenderLista() {
    const corpo = document.getElementById('atv-lista-corpo');
    if (!corpo) return;
    const tarefas = App._atvTarefasFiltradas();
    if (App._atvListaSub === 'cards')       corpo.innerHTML = App._atvListaCardsHTML(tarefas);
    else if (App._atvListaSub === 'tabela') corpo.innerHTML = App._atvListaTabelaHTML(tarefas);
    else if (App._atvListaSub === 'grupos') corpo.innerHTML = App._atvListaAgrupadaHTML(tarefas, 'categoria', State.atvCategorias, 'Sem categoria');
    else if (App._atvListaSub === 'setores') corpo.innerHTML = App._atvListaAgrupadaHTML(tarefas, 'setor', State.units, 'Sem setor');
  },
  _atvListaCardsHTML(tarefas) {
    if (!tarefas.length) return '<div class="atv-lista-empty">Nenhuma tarefa encontrada.</div>';
    return `<div class="atv-lista-cards-grid">${tarefas.map(t => App._atvCardComStatusHTML(t)).join('')}</div>`;
  },
  _atvListaTabelaHTML(tarefas) {
    if (!tarefas.length) return '<div class="atv-lista-empty">Nenhuma tarefa encontrada.</div>';
    const { campo, dir } = App._atvOrdenacaoTabela;
    const valorOrdenacao = (t, c) => {
      if (c === 'setor')       return (State.units && State.units[t.setor]) || '';
      if (c === 'categoria')   return (State.atvCategorias && State.atvCategorias[t.categoria]) || '';
      if (c === 'responsavel') return App._atvNomeAdmin(t.responsavel);
      if (c === 'status')      return (State.atvColunas && State.atvColunas[t.colunaId] && State.atvColunas[t.colunaId].nome) || '';
      return t[c] || '';
    };
    const ordenadas = [...tarefas].sort((a, b) =>
      String(valorOrdenacao(a, campo)).localeCompare(String(valorOrdenacao(b, campo))) * (dir === 'asc' ? 1 : -1));
    const th = (c, lbl) => `<th onclick="App._atvOrdenarTabela('${c}')">${lbl}${campo === c ? (dir === 'asc' ? ' ▲' : ' ▼') : ''}</th>`;
    return `<table class="atv-lista-table">
      <thead><tr>${th('titulo', 'Título')}${th('status', 'Status')}${th('setor', 'Setor')}${th('categoria', 'Categoria')}${th('responsavel', 'Responsável')}${th('inicio', 'Início')}${th('fim', 'Conclusão')}<th></th></tr></thead>
      <tbody>${ordenadas.map(t => {
        const col = State.atvColunas && State.atvColunas[t.colunaId];
        return `<tr onclick="App.atvAbrirTask('${t.id}')">
          <td>${t.titulo}</td>
          <td>${col ? `<span class="atv-lista-status-pill" style="background:${col.cor}22;color:${col.cor}">${col.nome}</span>` : '—'}</td>
          <td>${(State.units && State.units[t.setor]) || '—'}</td>
          <td>${(State.atvCategorias && State.atvCategorias[t.categoria]) || '—'}</td>
          <td>${App._atvNomeAdmin(t.responsavel) || '—'}</td>
          <td>${App._atvFmtData(t.inicio) || '—'}</td>
          <td>${App._atvFmtData(t.fim) || '—'}</td>
          <td><div class="atv-table-acts">
            <button onclick="event.stopPropagation();App.atvAbrirTask('${t.id}')" title="Editar">${App._svg('pencil')}</button>
            <button onclick="event.stopPropagation();App._atvExcluirDireto('${t.id}')" title="Excluir">${App._svg('trash')}</button>
          </div></td>
        </tr>`;
      }).join('')}</tbody>
    </table>`;
  },
  _atvOrdenarTabela(campo) {
    if (App._atvOrdenacaoTabela.campo === campo) App._atvOrdenacaoTabela.dir = App._atvOrdenacaoTabela.dir === 'asc' ? 'desc' : 'asc';
    else App._atvOrdenacaoTabela = { campo, dir: 'asc' };
    App.atvRenderLista();
  },
  _atvListaAgrupadaHTML(tarefas, campo, dict, semLabel) {
    if (!tarefas.length) return '<div class="atv-lista-empty">Nenhuma tarefa encontrada.</div>';
    const grupos = {};
    tarefas.forEach(t => { const chave = t[campo] || '__sem__'; (grupos[chave] = grupos[chave] || []).push(t); });
    const chaves = Object.keys(grupos).sort((a, b) => {
      const na = a === '__sem__' ? semLabel : ((dict && dict[a]) || a);
      const nb = b === '__sem__' ? semLabel : ((dict && dict[b]) || b);
      return String(na).localeCompare(String(nb));
    });
    return chaves.map(chave => {
      const nome = chave === '__sem__' ? semLabel : ((dict && dict[chave]) || chave);
      const gid = 'atvgrp_' + chave.replace(/[^a-zA-Z0-9]/g, '');
      return `<div class="atv-group">
        <div class="atv-group-header" onclick="this.classList.toggle('collapsed');document.getElementById('${gid}')?.classList.toggle('collapsed')">
          <svg viewBox="0 0 24 24" fill="none" width="14" height="14"><polyline points="6 9 12 15 18 9" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          <span class="atv-group-nome">${nome}</span>
          <span class="atv-group-count">${grupos[chave].length}</span>
        </div>
        <div class="atv-group-body" id="${gid}">${grupos[chave].map(t => App._atvCardHTML(t)).join('')}</div>
      </div>`;
    }).join('');
  },

  /* ── CALENDÁRIO (estilo Google — mensal/semanal) ── */
  atvCalNav(dir) {
    const ref = App._atvCalRef;
    if (App._atvCalModo === 'mes') ref.setMonth(ref.getMonth() + dir);
    else ref.setDate(ref.getDate() + dir * 7);
    App.atvRenderCalendario();
  },
  atvCalHoje() { App._atvCalRef = new Date(); App.atvRenderCalendario(); },
  atvCalSetModo(modo) {
    App._atvCalModo = modo;
    document.querySelectorAll('.atv-cal-modo .atv-sub-btn').forEach(b => b.classList.toggle('active', b.dataset.modo === modo));
    document.getElementById('atv-cal-grid')?.classList.toggle('modo-semana', modo === 'semana');
    App.atvRenderCalendario();
  },
  atvRenderCalendario() {
    const grid = document.getElementById('atv-cal-grid');
    const titulo = document.getElementById('atv-cal-titulo');
    if (!grid || !titulo) return;
    const ref = App._atvCalRef;
    const porDia = {};
    App._atvTarefasFiltradas().filter(t => t.fim).forEach(t => { (porDia[t.fim] = porDia[t.fim] || []).push(t); });

    const MESES = ['janeiro', 'fevereiro', 'março', 'abril', 'maio', 'junho', 'julho', 'agosto', 'setembro', 'outubro', 'novembro', 'dezembro'];
    const DOW = ['SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SÁB', 'DOM'];
    const weekdaysHTML = `<div class="atv-cal-weekdays">${DOW.map(d => `<span>${d}</span>`).join('')}</div>`;

    let dias = [];
    if (App._atvCalModo === 'mes') {
      titulo.textContent = `${MESES[ref.getMonth()]} ${ref.getFullYear()}`;
      const primeiro = new Date(ref.getFullYear(), ref.getMonth(), 1);
      const offset = (primeiro.getDay() + 6) % 7; // semana começando na Segunda
      const inicio = new Date(primeiro); inicio.setDate(inicio.getDate() - offset);
      for (let i = 0; i < 42; i++) { const d = new Date(inicio); d.setDate(d.getDate() + i); dias.push(d); }
    } else {
      const offset = (ref.getDay() + 6) % 7;
      const inicio = new Date(ref); inicio.setDate(inicio.getDate() - offset);
      for (let i = 0; i < 7; i++) { const d = new Date(inicio); d.setDate(d.getDate() + i); dias.push(d); }
      titulo.textContent = `${dias[0].getDate()}/${dias[0].getMonth() + 1} — ${dias[6].getDate()}/${dias[6].getMonth() + 1}`;
    }

    const hojeStr = new Date().toISOString().slice(0, 10);
    const maxMostrar = App._atvCalModo === 'semana' ? 20 : 3;
    const diasHTML = dias.map(d => {
      const iso = new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 10);
      const outroMes = App._atvCalModo === 'mes' && d.getMonth() !== ref.getMonth();
      const eventos = porDia[iso] || [];
      const mostrar = eventos.slice(0, maxMostrar);
      const resto = eventos.length - mostrar.length;
      return `<div class="atv-cal-day${outroMes ? ' outro-mes' : ''}${iso === hojeStr ? ' hoje' : ''}">
        <span class="atv-cal-day-num">${d.getDate()}</span>
        ${mostrar.map(t => `<span class="atv-cal-evento" title="${t.titulo}" onclick="App.atvAbrirTask('${t.id}')">${t.titulo}</span>`).join('')}
        ${resto > 0 ? `<span class="atv-cal-mais">+${resto} mais</span>` : ''}
      </div>`;
    }).join('');

    grid.innerHTML = weekdaysHTML + `<div class="atv-cal-days">${diasHTML}</div>`;
  }
});
